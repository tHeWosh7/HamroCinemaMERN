...
const Navbar = () => {
    ...
    return (
      <div className='fixed top-2 left-0 z-50 w-full flex items-center justify-between px-6 md:px-16 lg:px-36 py-2 -mx-5 md:-mx-20 md:-my-0'>
        <div>
            <Link to='/' className='max-md:flex-1'>
            <img src={assets.MainLogo} alt='logo' className='w-60 h-auto' />
            </Link>
        </div>
        <div/>
        <div className={`max-md:absolute max-md:top-0 max-md:left-0  max-md:font-medium
            ...
            min-md:rounded-full backdrop-blur bg-black/70 md:bg-navbarbgg/5 md:border
        ...
        <div className='flex items-center gap-8'>
            <SearchIcon className='max-md:hidden w-8 h-8 cursor-pointer text-red-500' />
            {
                !user ? (
                    <button onClick={openSignIn} className='px-4 py-1 sm:px-5 sm:py-2 bg-primary md:hover:bg-white 
                    md:border-1 hover:text-primary border:white border-white transition 
                    rounded-full font-medium cursor-pointer'>Log In</button>
                ) : (
                    <UserButton> 
